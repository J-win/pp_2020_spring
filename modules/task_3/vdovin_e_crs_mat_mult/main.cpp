...
/*TEST(CRSMatrix, test_build) {
    ...
    a.getThreads(1);
    double start1_time = omp_get_wtime();
    a * b;
    double end1_time = omp_get_wtime();
    std::cout << "Lin time: " << (end1_time - start1_time) << std::endl;

    a.getThreads(4);
    double start2_time = omp_get_wtime();
    a * b;
    double end2_time = omp_get_wtime();
    std::cout << "Par time: " << (end2_time - start2_time) << std::endl;
