...
#include <utility>
#include <cmath>
#include "../../../modules/task_3/vdovin_e_crs_mat_mult/crs_mat_mult.h"
...
CRSMatrix CRSMatrix::multiplicate(const CRSMatrix &mtx) const {
    if (n == mtx.n) {
        tbb::task_scheduler_init init(omp_k);
...
void Multiplicator::operator() (const tbb::blocked_range<int>& r) const {
    int begin = r.begin();
    int end = r.end();

    std::vector<int> tmp(a->n + 1);

    for (int i = begin; i < end; ++i) {
        std::fill(tmp.begin(), tmp.end(), -1);

        int ks = a->rowindex[i];
        int kf = a->rowindex[i + 1];
        ...
        if (ks != kf) {
            for (int j = 0; j < a->n; ++j) {
                ...
                int ls = b->rowindex[j];
                int lf = b->rowindex[j + 1];

                for (int k = ls; k < lf; ++k) {
                    int ind = tmp[b->col[k]];
                    if (ind != -1) {
                        sum += a->value[ind] * b->value[k];
                    }
                }
